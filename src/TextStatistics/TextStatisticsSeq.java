 ...
 * @author SevenThree
 */
public class TextStatisticsSeq implements TextStatistics {
    ...
    public String[] allTokensSorted() {
        return Arrays.stream(splitArray).sorted().toArray(String[]::new);
    }

    @Override
    public long tokensCount() {
        long count = Arrays.stream(splitArray).count();
        wordCount = count;
        return count;
    }
    ...
    public long uniqueTokensCount() {
        long count = Arrays.stream(splitArray).distinct().count();
        return count;
    }

    @Override
    public String[][] countsByTokens() {
        String[][] countByTokenArray = Arrays.stream(splitArray).collect(Collectors.groupingBy(Function.identity(), Collectors.reducing(
                0,
                o -> 1,
                Integer::sum))).entrySet().stream().map((i) -> new String[]{i.getKey(), i.getValue().toString()}).toArray(String[][]::new);
        return countByTokenArray;
    }

    @Override
    public Map<String, String> tokenPos() {
        return IntStream.range(0, splitArray.length).mapToObj(i -> Integer.toString(++i))
                .collect(Collectors.groupingBy(i -> splitArray[Integer.parseInt(i) - 1], Collectors.joining(",")));
    }
    ...
    public BigDecimal tokenTF(String name, int digits) {
        ...
        long tokenCount = Arrays.stream(splitArray).filter(i -> i.equals(name)).count();
    ...
    public Map<String, BigDecimal> everyTF(int digits) {
        ...
        Map<String, BigDecimal> TfMap = Arrays.stream(splitArray).collect(Collectors.groupingBy(Function.identity(),
